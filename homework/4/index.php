...
// !!! продумать вариант, если макс и мин стоят рядом или одинаковые // abs($imax - $imin == 1 || $max==$min
//$arr = [8, 8, 8, 8];
$max = $arr[0];
$min = $arr[0];
$max_index= 0;
...
for ($i = 0, $j = 1; $i < count($arr); $i += 2, $j += 2) {
    ...
    if ($j < count($arr)) {                            // чтобы не бало проверки лучше два цикла наверное
...
for ($i = 0, $k = 1; $i < count($arr_d); $i++, $k++) { // k - номер столбца  //  можно было $i + 1
